...
FACILITYS_FILE = DATA_DIR / "facilities.json"
FACILITY_TEMPLATE_FILE = Path("facilities_template.json")

if not FACILITYS_FILE.exists():
    DATA_DIR.mkdir(parents=True, exist_ok=True)

    if FACILITY_TEMPLATE_FILE.exists():
        shutil.copy(FACILITY_TEMPLATE_FILE, FACILITYS_FILE)

with open(FACILITYS_FILE, "r", encoding="utf-8") as f:
...
STATUS_OPTIONS = ["In Progress", "Submitted", "Status/Outcome", "Discharge"]
